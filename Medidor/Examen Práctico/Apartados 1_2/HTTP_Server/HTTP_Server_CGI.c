...
extern osThreadId_t TID_hilo2;
extern osThreadId_t TID_Barrido;
extern osThreadId_t tid_ThreadModo;
...
void netCGI_ProcessData (uint8_t code, const char *data, uint32_t len) {
  ...
  do {
    ...
    if (var[0] != 0) {
      ...
      if (strcmp (var, "led0=on") == 0) {
        ...
        P2 |= 0x04;				
			...
			/****************************************************************************************************************************************************************/
			///////////////PARA LOS CHECKBOX, SE AÑADEN AQUI LOS CASOS NUEVOS QUE QUERAMOS CON EL NOMBRE DEL BOTON QUE TENGAMOS EN EL CGI Y SERIA CON EL CASE B Y A PARTIR DEL 8/////////////////////
		  else if (strcmp (var, "activar0=on") == 0) 
			{
        o = 1;
				osDelay(50);
      }
			
			else if (strcmp (var, "activar1=on") == 0) 
			{
				if(bar == 0)
				{
					bar = 1;
			...
			///////////////PARA LOS DESPLEGABLES, DECLARAR AQUI LAS OPCIONES QUE VA A TENER CON EL MISMO NOMBRE QUE EN EL cgi/////////////////////
			else if (strcmp (var, "ctrl=OPCION1") == 0) 
			{
        u = 1;
      }
			else if (strcmp (var, "ctrl=OPCION2") == 0) 
			{
        u = 0;
      }
			/****************************************************************************************************************************************************************/
    ...
    LED_SetOut_stm(P2); //INVOCACION DE LED_SetOut para evaluar el dato recibido.
}
...
uint32_t netCGI_Script (const char *env, char *buf, uint32_t buflen, uint32_t *pcgi) {
  ...
  switch (env[0]) {
    ...
    /**********************************************************************************************************
     * EXPLICACION: se declara un nuevo caso para mostrar la hora del RTC. Se pasa por parametro tanto        *
     * la hora como la fecha que se actualizan en la funcion "void RTC_setTime(void)" del archivo "rtc.c"     * 
     **********************************************************************************************************/
			
    /**************************************************************************************************************
     * EXPLICACION CGI: se utiliza para este caso el archivo "buttons.cgi", que estaba inutilizado, se muestran   *
     * las dos variables (fecha y hora), y mediante t "<META http-equiv="refresh" content="1">" se consigue que   *
     * ambas se refresquen cada segundo. Se ha utilizado como plantilla el cgi del LCD.                           *
     **************************************************************************************************************/      
		...
		case 'm':
			...
			switch (env[2])
			{
				case '1':
					...
					len = sprintf (buf, &env[4], datosLuz);
      ...
      break;			
		...
		/****************************************************************************************************************************************************************/
		///////////////PARA EL DESPLEGABLE AGREGAMOS ESTE case 'n' Y SE COMPRUEBA QUE OPCION SE HA ESCOGIDO/////////////////////			
		/*case 'n':

      if (env[2] == 'd') {
        // Select Control
        len = (uint32_t)sprintf (buf, &env[4], u ?     ""     : "selected",
                                               u ? "selected" :    ""     );
        break;
      }		
			break;*/
